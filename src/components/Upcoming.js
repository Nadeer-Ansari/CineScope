...
export default function Upcoming() {
  ...
  useEffect(() => {
    ...
    const fetchUpcomingMovies = async () => {
      setLoading(true);
      let apipath = `https://api.themoviedb.org/3/movie/upcoming?api_key=${API_KEY}&language=en-US&page=1`;
  ...
  if (loading) {
    return (
      <div className="container text-center" style={{ padding: "4rem" }}>
        ...
        <h3 className="mt-3">Loading upcoming movies...</h3>
      </div>
    );
  }
    ...
    <div className="container">
      ...
      {data.length === 0 ? (
        <div className="text-center" style={{ padding: "3rem" }}>
          <p>No upcoming movies found. Please try again later.</p>
        </div>
      ) : (
        <div className="row">
