...
export default function Popular() {
  ...
  useEffect(() => {
    ...
    const fetchPopularMovies = async () => {
      setLoading(true);
      let apipath = `https://api.themoviedb.org/3/movie/popular?api_key=${API_KEY}&language=en-US&page=1`;
  ...
  if (loading) {
    return (
      <div className="container text-center" style={{ padding: "4rem" }}>
        ...
        <h3 className="mt-3">Loading popular movies...</h3>
      </div>
    );
  }
    ...
    <div className="container">
      ...
      {data.length === 0 ? (
        <div className="text-center" style={{ padding: "3rem" }}>
          <p>No popular movies found. Please try again later.</p>
        </div>
      ) : (
        <div className="row">
